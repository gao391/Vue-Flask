      ...
      <el-container>
        ...
        <el-main style="padding: 20px;">
          ...
          <el-table :data="tableData" style="margin-top: 20px;" ref="table" @selection-change="handleSelectionChange" :row-key="row => row.billNumber">
              ...
              <el-table-column prop="productInfo" label="房产信息"></el-table-column>
              <el-table-column prop="state" label="审核状态"></el-table-column>
              <el-table-column prop="number" label="房产数量"></el-table-column>
              <el-table-column prop="purchasingagent" label="采购人员"></el-table-column>
        ...
        <!-- 添加数据表单项 -->
          ...
          <el-form-item label="交易金额" prop="money">
            <el-input v-model="addFormData.money" placeholder="请输入交易的金额"></el-input>
          </el-form-item>
          ...
          <el-form-item label="商品数量" prop="number">
            <el-input v-model="addFormData.number" placeholder="请输入商品数量"></el-input>
          </el-form-item>
          <el-form-item label="采购员" prop="purchasingagent">
            <el-radio-group v-model="addFormData.purchasingagent">
              <el-radio label="小王">小王</el-radio>
              <el-radio label="小张">小张</el-radio>
              <el-radio label="小文">小文</el-radio>
              <el-radio label="小丽">小丽</el-radio>
            </el-radio-group>
          </el-form-item>
        <el-form-item>
          <!-- 提交和取消按钮 -->
  ...
  export default {
    data() {
      ...
      // 查询数据
        ...
        // 死数据
        tableData: [
        { money:'30000000',purchasingagent:'小王',number:'10',billNumber: '123', startDate: '2022-01-01', endDate: '2022-08-05', productInfo:'汤城一品', state:'通过' },
        { money:'12430000',purchasingagent:'小丽',number:'13',billNumber: '456', startDate: '2022-02-01', endDate: '2022-03-05', productInfo: '碧桂园', state:'未通过' },
        { money:'12330000',purchasingagent:'小张',number:'16',billNumber: '789', startDate: '2022-03-01', endDate: '2022-06-05', productInfo: '山水豪城', state:'未通过' },
      ],
      // 勾选框
      selectedRows: [],
      };
    },
    // 访问页面刷新数据库中的数据展示在页面当中
    created() {
    this.loadData();
    },
    methods: {
    ...
    handleSubmit() {
      ...
      this.$axios.post('/purchasing/add', {
          ...
          purchasingagent: this.addFormData.purchasingagent,
    ...
    this.$msgbox({
        ...
        .then(action => {
            if (action === 'confirm') {
                // Send a request to the backend to delete the data
                this.$axios.post('/purchasing/delete', {
    ...
    handleReview() {
      ...
      .then(action => {
        if (action === 'confirm') {
          ...
          // 向后端发送请求，更新数据库中的数据状态
          this.$axios.post('/purchasing/updateState', {
